...
1.  **5개 분야 탭 (`st.tabs`)**: 선생님들이 항목별로 혼동하지 않고 작업할 수 있도록 화면을 분리했습니다.
2.  **활동명(날짜) 자동 서식**: 
    * 자율활동과 진로활동 탭에는 `st.date_input`을 넣어 달력에서 날짜를 선택할 수 있게 했습니다.
    * AI에게 **"문장 시작 형식을 반드시 '활동이름(2025.09.23.)'로 시작할 것"**이라고 강제 지시하여 선생님이 원하는 서식을 정확히 지키도록 했습니다.
3.  **글자 수 조절 슬라이더**: 사이드바에서 글자 수를 조절하면 AI가 해당 분량에 맞춰 문장을 요약하거나 확장합니다.
4.  **2026 지침 내장**: PDF 전체를 읽는 대신, 제가 요약해 드린 핵심 지침을 `GUIDELINE_SUMMARY` 상수에 넣어 AI가 매번 참고하게 했습니다. 덕분에 속도가 매우 빠릅니다.

---

### 3. 배포를 위한 `requirements.txt` 내용
GitHub에 올리실 때 이 파일도 꼭 같이 만들어주세요.
